...
class NeuralNetwork:
    ...
    def _gradJ(self,X,Y,regularization_parameter ):
        ...
        #We stock the partial derivative in a datastructure of exact same format as self.W
    ...
    def predict(self,X):
        ...
        return np.array([ 1 if self._output(x) > 0.5 else 0 for x in X ])
...
X_train, X_test, y_train, y_test = gen_dataset( np.logical_xor, 2 )
